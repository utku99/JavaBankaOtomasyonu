...
public class PersonelGUI extends JFrame {
	 ...
	 */
	public PersonelGUI(Müþteri müþteri) throws SQLException, ClassNotFoundException {
		...
		for (int i = 0; i < kart.getList().size(); i++) {
			...
			kartData[3] = kart.getList().get(i).getType();
		...
		lblType_1.setBounds(697, 83, 89, 25);
		panel_2.add(lblType_1);

		textField_7 = new JTextField();
		textField_7.setColumns(10);
		textField_7.setBounds(549, 254, 148, 25);
		...
		comboBox_2.setBounds(697, 119, 138, 24);
		panel_2.add(comboBox_2);
		
		JButton button_7 = new JButton("Ekle");
		button_7.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (id.getText().length() == 0 || num.getText().length() == 0 || pass.getText().length() == 0) {
					Helper.showMsg("fill");
				} else {
					try {
						boolean control = kart.add(Integer.parseInt(id.getText()), num.getText(), pass.getText(), comboBox_2.getSelectedItem().toString());
						if (control) {
							Helper.showMsg("succes");
							id.setText(null);
							num.setText(null);
		...
		button_7.setBounds(549, 169, 286, 38);
		...
		button_8.setBounds(549, 290, 286, 37);
		panel_2.add(button_8);

		JScrollPane scrollPane_4 = new JScrollPane();
		scrollPane_4.setBounds(10, 46, 529, 281);
		...
		JLabel lblNotHerMteri = new JLabel("NOT:  Her m\u00FC\u015Fteri i\u00E7in 1 adet fatura id bulunmal\u0131d\u0131r !!!");
		...
		JLabel lblNotHerMteri_1 = new JLabel("NOT: Her m\u00FC\u015Fteri i\u00E7in 1 adet cep id bulunmal\u0131d\u0131r !!!");
		...
		JLabel lblNewLabel = new JLabel("NOT: \u015Eifre g\u00FCncellenebilir");
		lblNewLabel.setFont(new Font("Arial", Font.BOLD, 13));
		lblNewLabel.setBounds(10, 11, 242, 25);
		...
		table_4.getModel().addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				if (e.getType() == TableModelEvent.UPDATE) {
					...
					try {
						boolean control = kart.update(selectKartNumara, selectÞifre);
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					} catch (ClassNotFoundException e1) {
		...
		JLabel lblNotNumaraGncellenebilir = new JLabel("NOT: Numara g\u00FCncellenebilir");
		lblNotNumaraGncellenebilir.setFont(new Font("Arial", Font.BOLD, 13));
		lblNotNumaraGncellenebilir.setBounds(368, 11, 242, 25);
		panel_4.add(lblNotNumaraGncellenebilir);
		...
		JLabel lblNotHesapId_1 = new JLabel(
				"NOT: Hesap id, fatura id, telefon id ve kart id müþteri id ile ayný olmalýdýr.");
	...
	public void updateMüþteriModel() throws SQLException, ClassNotFoundException {
	...
	public void updateHesapModel() throws SQLException, ClassNotFoundException {
		DefaultTableModel clearModel = (DefaultTableModel) table.getModel();
		clearModel.setRowCount(0);
		for (int i = 0; i < hesap.getList().size(); i++) {
			hesapData[0] = hesap.getList().get(i).getHesap_id();
	...
	public void updateFaturaModel() throws SQLException, ClassNotFoundException {
		DefaultTableModel clearModel = (DefaultTableModel) table.getModel();
		clearModel.setRowCount(0);
		for (int i = 0; i < fatura.getList().size(); i++) {
			faturaData[0] = fatura.getList().get(i).getFatura_id();
	...
	public void updateCepModel() throws SQLException, ClassNotFoundException {
		DefaultTableModel clearModel = (DefaultTableModel) table.getModel();
		clearModel.setRowCount(0);
		for (int i = 0; i < cep.getList().size(); i++) {
			cepData[0] = cep.getList().get(i).getCep_id();
	...
	public void updateKartModel() throws SQLException, ClassNotFoundException {
		DefaultTableModel clearModel = (DefaultTableModel) table.getModel();
		clearModel.setRowCount(0);
		for (int i = 0; i < kart.getList().size(); i++) {
			kartData[0] = kart.getList().get(i).getKart_id();
